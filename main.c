 ...
 *
 * K philosophers seated around a circular table with one chopstick between
 * each pair of philosophers.
 *
 * There is only one chopstick between each philosopher.
 ...
 * 1. A mutex and
 * 2. A semaphore array for the philosophers.
 *
 * Mutex is used such that no two philosophers may access the
 * pickup or put down at the same time.
 *
